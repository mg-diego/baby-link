...
class DurationEventHandler {
  static StateNotifierProvider<ActiveEventNotifier, ActiveEvent?>? getProviderForEventType(EventType type) {
  ...
  static Future<void> handleAction({
    ...
    if (!isActive) {
      ...
      await _stopEvent(context, ref, eventType, provider, activeEvent, onSuccess);
    }
  }
            ...
            builder: (BuildContext context, StateSetter setModalState) {
              return Column(
                ...
                children: [
                  ...
                  Text('Iniciar ${eventType.uiLabel}', style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 20),
                  CustomTimePicker(
                    time: tempTime,
                    onTimeChanged: (newTime) => setModalState(() => tempTime = newTime),
                  ),
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      child: const Text('Guardar inicio', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                    ),
                  ),
                ],
              );
            ...
            builder: (BuildContext context, StateSetter setModalState) {
              ...
              return Column(
                ...
                children: [
                  ...
                  Text('Detener ${eventType.uiLabel}', style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
                  Padding(
                  ...
                  SizedBox(
                    width: double.infinity, height: 50,
                    child: ElevatedButton(
                      onPressed: isValid ? () => Navigator.pop(ctx, tempEndTime) : null,
                      style: ElevatedButton.styleFrom(
    ...
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('${eventType.uiLabel} finalizada correctamente')),
        );
      }
      onSuccess();
    } catch (e) {
  ...
  static String _formatTimeOnly(DateTime time) => '${time.hour.toString().padLeft(2, '0')}:${time.minute.toString().padLeft(2, '0')}';
